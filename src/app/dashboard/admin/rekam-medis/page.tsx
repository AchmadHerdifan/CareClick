...
export default function RekamMedisPage() {
  ...
  async function fetchData() {
    ...
    const [{ data: rows }, { data: pasiens }, { data: dokters }] = await Promise.all([
      supabase.from('rekam_medis').select('*, pasien(nama), dokter(nama)').order('tanggal', { ascending: false }),
      supabase.from('pasien').select('id, nama').order('nama'),
      supabase.from('dokter').select('id, nama').order('nama'),
    ])
    setData(rows || [])
  ...
  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault()
    await supabase.from('rekam_medis').insert(form)
    setShowModal(false)
    setForm({ pasien_id: '', dokter_id: '', tanggal: new Date().toISOString().split('T')[0], keluhan: '', diagnosis: '', resep: '', catatan: '' })
    fetchData()
  }

  async function handleDelete(id: string) {
    if (!confirm('Yakin hapus rekam medis ini?')) return
    await supabase.from('rekam_medis').delete().eq('id', id)
    fetchData()
  }
      ...
      {showModal && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-xl shadow-xl w-full max-w-lg p-6 max-h-[90vh] overflow-y-auto">
            <h2 className="text-lg font-bold text-gray-900 mb-4">Input Rekam Medis</h2>
            <form onSubmit={handleSubmit} className="space-y-3">
              ...
              <div>
                <label className="label">Resep</label>
                <textarea className="input-field" rows={2} value={form.resep} onChange={(e) => setForm({ ...form, resep: e.target.value })} />
              </div>
              <div>
